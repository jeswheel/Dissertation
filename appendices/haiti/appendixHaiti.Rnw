...
\begin{figure}[ht]
<<m1Profs, fig.height=5, fig.width=6, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center'>>=
...
\hline
  ...
  Process noise {\footnotesize (wk\textsuperscript{1/2})} & $\sigmaProc: \mathrm{Epi}$ & $\Sexpr{myround(h1_mles %>% filter(prof_var == 'sig_sq_epi') %>% pull(value), 3)}$
   &
...
\hline
  ...
  Process noise {\footnotesize (wk\textsuperscript{1/2})} & $\sigmaProc: \mathrm{End}$ & $\Sexpr{myround(h1_mles %>% filter(prof_var == 'sig_sq_end') %>% pull(value), 3)}$
   &
...
\begin{figure}[ht]
<<m2Profs, fig.height=3, fig.width=5.5, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center'>>=
cowplot::plot_grid(
...
\begin{figure}[ht]
<<m3Profs, fig.height=4.6, fig.width=5.8, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center'>>=
h3_plots <- list()
for (i in 1:length(prof_params)) {
  p = prof_params[i]
  if (p == 'mu_B') {
...
\hline
 ...
 Process Noise {\footnotesize (wk\textsuperscript{1/2})} & $\sigmaProc$ & $\Sexpr{myround((h3_mles %>% filter(prof_var == 'std_W') %>% pull(value)) * sqrt(52), 3)}$
   &
  $(\Sexpr{myround((all_ci %>% filter(prof_var == 'std_W') %>% pull(lower)), 3)}, \Sexpr{myround((all_ci %>% filter(prof_var == 'std_W') %>% pull(upper)), 3)})$
\\
\hline

 Water Survival {\footnotesize (wk)} & $\Wremoval^{-1}$ & $\Sexpr{myround(52 / (h3_mles %>% filter(prof_var == 'mu_B') %>% pull(value)), 3)}$
   &
  $(\Sexpr{myround(1 / (all_ci %>% filter(prof_var == 'mu_B') %>% pull(upper)), 3)}, \Sexpr{myround(1 / (all_ci %>% filter(prof_var == 'mu_B') %>% pull(lower)), 3)})$
\\
\hline

 Human to Water Shedding {\footnotesize $\frac{\mathrm{km^2}}{\mathrm{wk}}$} & $\Wshed$ & $\Sexpr{signif((h3_mles %>% filter(prof_var == 'thetaI') %>% pull(value))/52, 3)}$
   &
...
Simulations from replicated Model 1. Parameter sets were generated by running source code provided by \citet{lee20}. Compare to Figure~S7 in the supplement of \citet{lee20}. The upper bound for the likelihood of this model is \Sexpr{round(mod1_lee_ll)}.
}
\end{figure}
...
Replicated Model~2 trajectories. The original model was re implemented using the \code{haitipkg}. Compare to Figure S15 in the supplement of \citet{lee20}.
}
\end{figure}
...
\begin{figure}[!ht]
<<h3UnitLikes, fig.height=3, fig.width=5.5, echo=FALSE,fig.align='center'>>=
...
\begin{figure}[!ht]
<<finalUnitLLFig, fig.height=3, fig.width=5.5, echo=FALSE, fig.align='center'>>=
