...
The general ARMA model was first described by \citet{whittle51}, and popularized by \citet{box1970}.
...
In this chapter, we address all three possible explanations by demonstrating the existence of an existing shortcoming, explaining why the problem has nontrivial consequences, and proposing a readily applicable and computationally efficient solution.
...
In this chapter, we focus on the standard implementations in R (\code{stats} package) and Python (\code{statsmodels} module), which we selected due to their widespread usage.
...
The challenges in parameter estimation arise in this situation because there is no closed-form expression for the likelihood function, though computational algorithms do exist for maximizing ARMA likelihoods \citep{gardner1980}.

We begin by providing essential background information on the estimation of ARMA model parameters.
We then present our proposed approach for parameter estimation, which leads to parameter values with a likelihood that is never lower and sometimes higher than currently accepted standards.
...
Throughout the chapter, we use the \code{stats::arima} function from the R programming language as a baseline for comparison.
The same methodology for fitting parameters is used in the \code{statsmodels.tsa} module in Python, and we demonstrate that our results apply to this software as well (Appendix~\ref{sec:appendix_arima_python}).

\section{Maximum Likelihood for ARMA Models}

A time series $\{y_n; n = 0, \pm1, \pm2, \ldots\}$ is said to be $\text{ARMA}(p, q)$ if it is (weakly) stationary and
\begin{equation}
  y_n = \phi_1y_{n - 1} + \cdots + \phi_py_{n - p} + w_n + \theta_1w_{n-1} + \ldots + \theta_qw_{n - q}, \label{eq:arma}
\end{equation}
...
A non-zero intercept could also be added to Eq.~\myeqref{eq:arma}, but for simplicity we assume that the time series $\{y_n; n = 0, \pm1, \pm2, \ldots\}$ has zero mean.
...
For instance, parameters can be estimated through methods such as Bayesian inference \citep{monahan83,chib94} or neural networks \citep{chon97}, among many others.
Specialized methods include those for integer-valued data \citep{weiss24}, approaches robust to outliers \citep{chakhchoukh10}, and non-Gaussian ARMA processes \citep{lii90}.
While each methodology has its merits, we focus on the approach most widely adopted in statistical practice: likelihood maximization, under the assumption that the WN process $\{w_n\}$ is Gaussian.
...
By solving for the WN term $w_n$, Eq.~\myeqref{eq:arma} can be written as
\begin{equation}
w_n = y_n - \sum_{i = 1}^p \phi_i y_{n - i} - \sum_{j = 1}^q \theta_j w_{n-j}. \label{eq:css}
\end{equation}
A natural estimator would involve minimizing the sum of squares $\sum_{n = 1}^N w_n^2$.
However, since only $y_1, y_2, \ldots, y_N$ are observed and $w_{n}$ is recursively defined in Eq.~\myeqref{eq:css} using values of $y_{n-p}$ and $w_{n-q}$, directly minimizing this sum is intractable.
The CSS method addresses this issue by conditioning on the first $p$ values of the process, assuming $w_p = w_{p-1} = \ldots = w_{p+1-q} = 0$, and minimizing the conditioned sum $\sum_{n = p+1}^N w_n^2$.
...
CSS minimization was previously popular because methods for likelihood maximization were considered prohibitively slow, though this is no longer the case with currently available hardware and software \citep{ripley2002}.

For likelihood maximization, Eq.~\myeqref{eq:arma} is reformulated as an equivalent state space model.
Although there are several ways this can be done, the approach of \citet{gardner1980} is widely used.
...
We define a latent state vector $X_n \in \mathbb{R}^{r}$, along with transition matrices $T\in \mathbb{R}^{r\times r}$ and $Q \in \mathbb{R}^{r\times 1}$, enabling the recovery of the original sequence $\{y_n\}$ using Equations~\myeqref{eq:trans} and \myeqref{eq:meas}.
For a detailed explanation of how to define the latent state $X_n$ and transition matrices $T$ and $Q$ in order to recover the ARMA model, we refer readers to \citet[Chapter~3]{durbin12}.
Along with initializations for the mean and variance of $X_0$, these equations complete the state space model and POMP specification introduced in Chapter~\ref{chpt:intro}.
Furthermore, as the latent process is defined using linear Gaussian updates, the likelihood function can be calculated exactly using the Kalman filter \citep{kalman60}.
The current standard approach for parameter estimation is to combine an Kalman filter evaluate with a numeric procedure such as the BFGS algorithm \citep{fletcher00} in order to maximize the likelihood.
\begin{align}
  X_n &= TX_{n - 1} + Qw_n, \label{eq:trans} \\
  y_n &= \begin{pmatrix} 1 & 0 & \ldots & 0\end{pmatrix} y_n. \label{eq:meas}
\end{align}
...
This is an effective approach because the CSS estimator asymptotically converges to the MLE \citep{shumway2017}, and may therefore be close to the global maximum when there are sufficiently many observations.
...
The log-likelihood function of ARMA models is often multimodal \citep{ripley2002}, and therefore this single initialization approach can result in parameter estimates corresponding to local maxima (see Figure~\ref{fig:multiMode}).
...
definitions are in \citet[Chapter~3]{shumway2017}.
Let $\{\phi_i\}_{i = 1}^p$ and $\{\theta_i\}_{i = 1}^q$ be the coefficients of the $\text{ARMA}(p, q)$ model (Eq.~\myeqref{eq:arma}), and define $\Phi(x) = 1 - \phi_1 x - \phi_2x^2 - \ldots - \phi_px^p$ as the AR polynomial, and $\Theta(x) = 1 + \theta_1 x + \theta_2x^2 + \ldots + \theta_qx^q$ as the MA polynomial.
...
\begin{algorithm}[ht]
   ...
   \caption[ARMA MLE]{\textbf{MLE for ARMA Models}. \\
    ...
    Numeric optimization routine $f(\psi)$ \citep{gardner1980}.
    \\\hspace{\textwidth}
...
The ARMA model (Eq.~\myeqref{eq:arma}) can be rewritten as:
\begin{equation}
  \Phi(B)x_t = \Theta(B)w_t, \label{eq:armaB}
\end{equation}
where $B$ is the \emph{backshift} operator, i.e., $Bx_t = x_{t - 1}$.
Using the fundamental theorem of algebra, Equation \myeqref{eq:armaB} can be factored into 
$$
...
As an elementary example, consider the $\text{ARMA}(1, 1)$ and $\text{ARMA}(2, 2)$ models in equations~\myeqref{eq:arma11} and \myeqref{eq:arma22}.
\begin{align}
...
\end{align}
While these two models appear distinct at first glance, re-writing the models in polynomial form (Eq.~\myeqref{eq:armaB}) shows that these two models are actually equivalent after canceling out the common factors on each side of the equation.
...
For this chapter, both the numeric optimization procedure $f(\cdot)$ and the parameter initialization strategy to obtain $\psi_0$ are those implemented in \code{stats::arima}.
...
Alternative stopping criterion can be used \citep[e.g.,][]{hart98}, but we found that this simple heuristic works well in practice.

Algorithm~\ref{alg:mle} is implemented in the R package \code{arima2}, available on the Comprehensive R Archive Network (CRAN) \citep{arima2Cran}.
...
The rate of improvement obtained using our algorithm increases with model complexity and decreases with more observations (Figure~\ref{fig:simProps}).
...
@
\caption[Proportion of simulated data with improved likelihood from using multiple restarts (Algorithm~\ref{alg:mle}).]{\label{fig:simProps}Proportion of simulated data with improved likelihood from using multiple restarts (Algorithm~\ref{alg:mle}).}
\end{figure}
...
Figure~\ref{fig:pctCovered} shows PLCIs had better or equivalent nominal coverage than Fisher-based confidence intervals across all combinations of $p$, $q$, and $n$. We found that the interval estimation method mattered more for confidence interval performance than the specific parameter estimation algorithm.
...
@
\caption[Proportion of models that achieved nominal coverage of Bonferroni adjusted $95\%$ confidence intervals.]{\label{fig:pctCovered}Proportion of models that achieved nominal coverage of Bonferroni adjusted $95\%$ confidence intervals. The dashed line denotes the target coverage level. (A) Confidence intervals created using Fisher's information matrix. (B) Confidence intervals created using profile likelihoods.}
\end{figure}
...
A contemporary approach involves fitting several candidate models and selecting the one that minimizes a criterion like Akaike's information criterion (AIC) \citep{aic74}. 
...
Evidence of improper optimization may be less evident when relying on software that automates this process, such as the automated Hyndman-Khandakar algorithm \citep{hyndman08}, but the potential for sub-optimal estimates remains.
...
This is illustrated in Figure~\ref{fig:AICtabRes}, where a non-zero percentage of AIC tables remain inconsistent, even as the algorithm's stopping criterion grows.
The ARMA$(1, 1)$ panel in Figure~\ref{fig:AICtabRes} illustrates the increasing difficulty of parameter estimation when dealing with parameter redundancies.
...
@
\caption[Proportion of consistent AIC tables.]{\label{fig:AICtabRes}Proportion of consistent AIC tables. Data is generated from ARMA$(p, q)$ models with $(p, q) \in \{1, 2, 3\}^2$, and the corresponding AIC table is created. The Y-axis shows the percentage of tables that were consistent. M is the number of times a maxima is observed before the algorithm terminates, so $M=1$ corresponds to the standard maximization procedure.}
\end{figure}
...
The use of AIC for ARMA model order selection has theoretical support, particularly for forecasting, as ARMA models inspired the original AIC paper \citep{aic74}.
...
We consider a dataset containing annual observations on the average depth of Lake Michigan-Huron, recorded the first day of each year from 1860-2014 (Figure~\ref{fig:huron}) \citep{NOAA16}.
We wish to develop an ARMA model for these data, which is a standard task in time series analysis \citep{shumway2017}.
...
Diagnostic tests, such as sample autocorrelation and normal quantile plots for residuals, suggest that it is reasonable to model the data in Figure~\ref{fig:huron} as a weakly stationary Gaussian $\text{ARMA}(p, q)$ process for some non-negative integers $p$ and $q$.
...
For example, a table of AIC values can be generated, and models with promising likelihoods can be explored further \citep{brockwell1991}.
...
\begin{table}
\centering
\caption{\label{tab:huronCoefsTab}Parameter values of an ARMA model fit to Lake Michigan-Huron data.}
\begin{tabular}{c|c|c|c|c}\hline
...
The $95\%$ PLCI for the parameter (Figure~\ref{fig:huronMAevid}A) is much larger than the confidence interval created using these standard errors.
...
Alternative evidence indicates the potential for nearly canceling roots (Figure~\ref{fig:huronRoots}), in which case the MA(1) term may not be needed in the model.
...
@
\caption[Inverted polynomial roots of ARMA models.]{\label{fig:huronRoots}Inverted polynomial roots of ARMA models. AR and MA polynomial roots to the fitted $\text{AR}(1)$ and $\text{ARMA}(2, 1)$ models to the Lake Michigan-Huron data using a single parameter initialization.}
\end{figure}
...
A histogram containing the estimated values of $\hat{\theta}_1$ when the data are generated from the $\text{ARMA}(2, 1)$ and $\text{AR}(1)$ models fit to the Lake Huron-Michigan data are shown in Figure~\ref{fig:huronMAevid}B and Figure~\ref{fig:huronMAevid}C, respectively.

The shape of this histogram in Figure~\ref{fig:huronMAevid}B mimics that of the profile log-likelihood surface in Figure~\ref{fig:huronMAevid}A, confirming that a large confidence interval is needed in order to obtain a $95\%$ confidence interval.
In Figure~\ref{fig:huronMAevid}C, a large number of $\hat{\theta}_1$ coefficients are estimated near $1$ when the generating model is $\text{AR}(1)$.
Combining this result with the nearly canceling roots of the $\text{ARMA}(2, 1)$ model (Figure~\ref{fig:huronRoots}), we cannot reject the hypothesis that the data were generated from a $\text{AR(1)}$ model, even though the Fisher information standard errors suggest that the data should be modeled with a nonzero $\theta_1$ coefficient.
...
Consequently, methodology relying on existing estimation methods---like the popular \code{auto.arima} function in the \code{forecast} package in R \citep{hyndman08}, which minimizes the AIC of a group of candidate models without explicitly displaying an AIC table---will be impacted by improved estimates.

Our proposed algorithm is supported by existing theory on likelihood evaluation of linear state space models via the Kalman Filter \citep{kalman60}, the same as the current existing standard approach for parameter estimation.
...
A common situation where our algorithm is expected to provide even larger improvements than those reported here is in the presence of missing data, a primary motivator of the likelihood maximization procedure of existing software \citep{ripley2002}.
...
Parameter estimates corresponding to higher likelihood values are not necessarily scientifically preferable to alternative regions of parameter space with lower likelihood values \citep{lecam1990}.
