...
ma1_res <- readRDS("data/sim_MA1_results.rds")
...
<<loadSimulation>>=
simulation1 <- readRDS("data/sim1_results.rds")
...
<<getTiming>>=
sim_times <- readRDS("data/sim1_timing.rds")
...
<<simTables>>=
simTables <- readRDS("data/sim2_AICtable_consistency.rds")
arma_inc <- simTables %>%
  filter(max_repeat == 1) %>%
