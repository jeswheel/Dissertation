...
root <- here::here(here::here(), "chapters", "arima2")
...
ma1_res <- readRDS(here::here(root, "data/sim_MA1_results.rds"))
...
<<loadSimulation>>=
simulation1 <- readRDS(here::here(root, "data/sim1_results.rds"))
...
<<getTiming>>=
sim_times <- readRDS(here::here(root, "data/sim1_timing.rds"))
...
<<simTables>>=
simTables <- readRDS(here::here(root, "data/sim2_AICtable_consistency.rds"))
arma_inc <- simTables %>%
  filter(max_repeat == 1) %>%
...
theta_21 <- bake(
  file = here::here(root, 'data/PLCIsims.rds'),
  {
...
theta_1 <- bake(
  file = here::here(root, 'data/ar1sims.rds'),
  {
