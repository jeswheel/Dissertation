\chapter{Informing Policy via Dynamic Models: Cholera in Haiti}
\label{chpt:haiti}

<<HaitiSetup, include=FALSE,echo=FALSE,results='hide'>>=
...
# Register parallel back-end. This code below creates a fork cluster, so we are assuming
# that the code is run on a mac or linux machine; this doesn't matter if you
# plan on simply loading the results, but if you would like to redo the expensive
# calculations (e.g., test minor changes to the code), you need to make sure you
# register the correct parallel back-end.
...
<<Calculate Lee1 Likelihoods, echo=FALSE, message=FALSE, include=FALSE>>=
h1_epi <- haiti1()

set.seed(141837)

lee_epi_ll <- readRDS(paste0(root, "data/model1/lee1_epi_evals.rds"))

# Create the endemic version of the model
h1_end <- haiti1(period = "endemic")

lee_end_ll <- readRDS(paste0(root, "data/model1/lee1_end_evals.rds"))
...
# Doing some clean-up. The goal is that each major chunk can be run independent
# of other chunks. Removing saved objects is not entirely necessary, but can
# help new users know what objects are needed later on and which were only
# temporarily needed for each particular chunk. Here we suppress warnings
# because some of the temporary objects only exist if the `bake` code needs to
# be recomputed, and hence `rm` will throw warnings if it tries to remove
# objects that don't exist.
suppressWarnings(
  rm(
    lee_end_ll, lee_epi_ll, h1_end, h1_epi, i, mod1_all_likes, end_params, 
    epi_params, pf_params, j, ll_evals, which_params
  )
)
gc()  # Garbage Collection to free un-used memory
@
...
% We assess current methodological approaches to these issues via a case study of the 2010-2019 cholera epidemic in Haiti.
...
% Specifically, we present approaches for diagnosing model misspecification and the consequent development of improved models.
% Additionally, we demonstrate the utility of recent advances in likelihood maximization for high-dimensional nonlinear dynamic models, enabling likelihood-based inference for spatiotemporal incidence data using this class of models.
...
% In this article, we conduct a re-analysis of the 2010-2019 cholera outbreak in Haiti. We use this case study to investigate current procedures for fitting mechanistic models to time series data, while identifying limitations of these methodologies and proposing remedies.
% Our analysis presents methodology for diagnosing how well a model describes observed data. Using objective measures to assess model fit ensures that our evaluation is based on quantifiable criteria. Incorporating reproducibility into this assessment results in a framework that enables the validation or refinement of model based inferences when revisiting the data, facilitating scientific discovery.
...
Quantitative models for these dynamic systems offer potential for designing effective control measures \citep{vandermeer13,he10}.
Developing and testing models for dynamic systems, and assessing their fitness for guiding policy, is a challenging statistical task \citep{king16}.
...
This case study investigates the use of dynamic models and spatiotemporal data to inform public health policy in the context of the cholera outbreak in Haiti, which started in 2010.
Various dynamic models were developed to study this outbreak: searching PubMed with keywords ``Haiti, cholera, model'' we obtained 22 studies that utilized deterministic mechanistic dynamic models \citep{lee20,tuite11,andrews11,botelho21,fitzgibbon20,eisenberg13,rinaldo12,chao11,date11,lin19,abrams13,collins21,akman15,trevisin22,mavian20,collins14,kelly16,capone15,leung22,mari15,gatto12,kuhn14}
and 11 studies that used stochastic models \citep{kirpich16,lee20,pasetto18,mukandavire13,kirpich17,lewnard16,kunkel17,mukandavire15,sallah17,azman12,azman15}.
Incidence data on the outbreak are available at various spatial scales, motivating 17 studies in our literature review to consider spatially explicit dynamic models \citep{lee20,tuite11,pasetto18,fitzgibbon20,eisenberg13,rinaldo12,chao11,abrams13,trevisin22,sallah17,collins14,kelly16,azman12,leung22,kuhn14,mari15,gatto12}.
Here we focus on a multi-group modeling exercise by \citet{lee20} in which four expert modeling teams developed models to the same dataset with the goal of comparing conclusions on the feasibility of eliminating cholera by a vaccination campaign.
...
These modeling strategies were selected by \citet{lee20} to represent the range of approaches used in the research community.
We focus on Models~1--3, as the strengths and weaknesses of the agent-based modeling approach \citep{tracy18} are outside the scope of this chapter.
In addition, agent-based models were less widely used, the agent based model in \citet{lee20} being the only model of this class that was found in our literature review.
The data that were used in \citet{lee20}, and that we reanalyze, are displayed in Figure~\ref{fig:CholeraData}.
...
@
\caption[Weekly reported cholera cases in Haiti from October 2010 to January 2019.]{\label{fig:CholeraData}
Weekly reported cholera cases in Haiti from October 2010 to January 2019. Data are displayed for each of the 10 administrative departments.
}
\end{figure}
...
The possibility of controlling the Haiti cholera outbreak via OCV was considered by various research groups \citep{lee20,ivers17,andrews11,walton11,matias17,chao11,fung13,date11,leung22,azman15}.
In the \citet{lee20} study, certain data were shared between the groups, including demography and vaccination history; vaccine efficacy was also fixed at a shared value between groups.
Beyond this, the groups made autonomous decisions on what to include and exclude from their models.
Despite their autonomy, the four independent teams obtained a consensus that an extensive nationwide vaccination campaign would be necessary to eliminate cholera from Haiti, estimating that a large number of cumulative cholera cases would be observed in the absence of additional vaccination efforts (Figure~3 and 4 of \citet{lee20}).
These forecasts are inconsistent with the prolonged period with no confirmed cholera cases between February, 2019 and September, 2022 \citep{trevisin22}.
Though cholera has recently reemerged in Haiti \citep{rubin22,PAHO23}, the inability to accurately forecast cholera incidence from 2019-2022 prompts us to consider retrospectively what may have been done differently in order to obtain more reliable conclusions, leading to recommendations for future studies.

The discrepancy between the model-based conclusions of \citet{lee20} and the prolonged absence of cholera in Haiti has been debated \citep[Lee et al., 2020a;][]{francois20,rebaudetComment20,henrys20}. \nocite{leeReply20}
Suggested origins of this discrepancy include the use of unrealistic models \citep{rebaudetComment20} and unrealistic criteria for cholera elimination \citep{henrys20}.
...
Numerous guidelines have been proposed for using mechanistic models to inform policy, reviewed by \citet{behrend20}, who identify the importance of stakeholder engagement, transparency, reproducibility, uncertainty communication, and testable model outcomes. 
These and related principles are echoed by other influential articles \citep{saltelli20,donnelly18}.
Additional literature emphasizes model calibration and evaluation techniques \citep{dahabreh17,egger17,penaloza15}. These guidelines often lack implementation specifics. As an example, \citet{lee20} largely adhere to the principles of \citet{behrend20}---though assessing the extent of stake-holder engagement is challenging---yet their projections are inconsistent with actual cholera incidence data from 2019 to 2022, demonstrating the limitations of current standards. We provide methodology for rigorous statistical calibration and evaluation of dynamic models (as advocated by \citet{saltelli19}), thereby expanding on the prevailing guidance. We specifically emphasize principles that prove essential in our case study. Complementary methodological suggestions arising from a spatio-temporal analysis of COVID-19 are detailed in \citet{li23}.

Our recommendations are presented in the context of a case study, with the goal of demonstrating how careful adherence to statistical principles may result in improved model fits.
...
Specifically, we focus on how to develop and test variations of the proposed models, as well as diagnosing the models once they have been assimilated to incidence reports.
This includes a comprehensive tutorial on performing inference with Model~3 (Appendix~\ref{sec:appendix_haiti_mod3Cal}), a highly non-linear, spatially explicit stochastic model, a challenging task that is possible due to recent methodological advancements.
We then use the improved model fits to project cholera incidence in Haiti under various vaccination scenarios considered by \citet{lee20}.
Finally, we conclude with a discussion of the results, in which we relate our general recommendations for model based inference of biological systems to the case study of the Haiti cholera outbreak.

<<Load Model1 Params, echo=FALSE, message=FALSE, include=FALSE>>=

h1_prof <- readRDS(paste0(root, "data/model1/h1_profiles.rds"))
...
h1_prof_res <- h1_prof %>%
  filter(prof_var == 'betat') %>%
  group_by(betat) %>%
  summarize(logLik = max(logLik))

# This is used later in figure 2
beta_t_mcap_results <- pomp::mcap(h1_prof_res$logLik, h1_prof_res$betat, span = 1)

# Find the set of parameters that correspond to the maximum likelihood
p1 <- h1_prof %>%
  slice_max(order_by = logLik, n = 1) %>%
  # Remove non-parameters from the data.frame (hyperparameters + likelihood)
  select(-(1:12)) %>%
  unlist()
...
<<Fit Model 2, echo=FALSE, include=FALSE, message=FALSE, warning=FALSE>>=

# Calculate log-likelihood of Lee et al (2020) model.
# We have parameter estimates for everything except the variance of the
# measurement model, so here we fix all parameters except the variance, which
# is estimated for both endemic and epidemic periods.
lee20_replication <- readRDS(paste0(root, "data/model2/lee20_replication.rds"))
h2_fit <- readRDS(paste0(root, "data/model2/model2_fit.rds"))
@

<<Load Model3 Parameters, echo=FALSE, include=FALSE, message=FALSE>>=

# Calculate the number of parameters that are estimated:
mod3_n_params <- length(c(
  "mu_B", "XthetaA", "thetaI", "lambdaR", "r", "std_W", "epsilon", "k",
  paste0('betaB', 1:10), paste0('foi_add', 1:10),
  'aHur3', 'aHur9',
  'hHur3', 'aHur9',
  'Iinit3', 'Iinit4'
))

h3_prof <- readRDS(paste0(root, "data/model3/h3_MLE_eval.rds"))
@

<<Create Model Objects,echo=FALSE,eval=TRUE, include=FALSE, message=FALSE>>=
...
load(paste0(root, "data/models.rda"))
@

<<Get AIC table Input, echo=FALSE, eval=TRUE>>=
mod2_ll <- h2_fit$logLik
...
mod3_eval_res <- readRDS(paste0(root, "data/model3/mod3_eval_res.rds"))

mod3_ll <- mod3_eval_res$ll[1]
mod3_aic <- 2 * mod3_n_params - 2 * mod3_ll
mod3_eval_res$subset_aic <- 2 * mod3_n_params - 2 * mod3_eval_res$subset_ll['se']
@

<<Calculate Bench Likelihood, echo=FALSE, message=FALSE, include=FALSE, cache=FALSE>>=
...
# Fit Model 1 benchmark:
m1_bench <- ar1_NegBinom(m1_agg_data$cases, init = 7364)
benchmarks[['m1']] <- m1_bench$ll

rm(m1_agg_data, m1_bench)
gc()
...
# This are initial values for Y_u(0). They were estimated, but because they are
# integers, close to the value of the first observation, it was easiest to fit these
# by hand.
m2_inits <- c(6449, 542, 0, 0, 16, 0, 0, 522, 0, 0)  # fit by hand
...
for (i in 1:10){
  ...
  m2_bench <- ar1_NegBinom(data = cases, init = m2_inits[i])

  m2_benchmark <- m2_benchmark + m2_bench$ll
}

benchmarks[['m2']] <- m2_benchmark

rm(m2_benchmark, h2_dep_names, cases, h2_data, m2_bench, i)
gc()
...
for (dep in 1:10) {
  ...
  m3_benchmark <- m3_benchmark + ar1_NegBinom(data = cases, init = m3_inits[dep])$ll
}

benchmarks[['m3']] <- m3_benchmark
rm(cases, m3_benchmark, dep, mod3_cases)
gc()
@

<<Computing AIC of Lee et al, echo=FALSE, include=FALSE, message=FALSE>>=
mod1_lee_n_params <- 20
mod1_lee_aic <- 2 * mod1_lee_n_params - 2 * mod1_lee_ll

mod2_lee_aic <- 2 * unname(lee20_replication['leeFit_n_params']) - 2 * unname(lee20_replication['leeFit_ll'])
...
lee3_results <- readRDS(paste0(root, 'data/model3/lee3_results.rds'))
...
Mechanistic models representing biological phenomena are valuable for epidemiology and consequently for public health policy \citep{lofgren14,mccabe21}. More broadly, they have useful roles throughout biology, especially when combined with statistical methods that properly account for stochasticity and nonlinearity \citep{may04}. In some situations, modern machine learning methods can outperform mechanistic models on epidemiological forecasting tasks \citep{lau22,baker18}. The predictive skill of non-mechanistic models can reveal limitations in mechanistic models, but cannot readily replace the scientific understanding obtained by describing the biological dynamics of the system in a mathematical model \citep{baker18,prosperi20}.

In this chapter, we refer to models that focus on learning relationships between variables in a dataset as {\it associative}, whereas models that incorporate a known scientific property of the system we call {\it causal} or {\it mechanistic}.
...
Despite their limited ability to project the effect of interventions on a system, associative models can be effectively used to make inference on an certain features of a system.
In our literature review, there were 14 studies that used associative models to describe various aspects of the cholera epidemic \citep[Richterman et al., 2019a, 2019b;][]{hulland19,moise20,piarroux11,matias17,eisenberg13,rebaudet19CATI,ivers15,raila17,cuneo17,charles14,bengtsson15,li16adjusting}. \nocite{richterman19a}\nocite{richterman19b}

The four mechanistic models of \citet{lee20} were deliberately developed with limited coordination.
...
Translations back to the original notation of \citet{lee20} are given in Table~\ref{tab:translate}.

\begin{table}
% \begin{adjustwidth}{-2.25in}{0in}
\centering
\footnotesize
\begin{tabular}{|p{0.165\linewidth}|p{0.16\linewidth}t|p{0.17\linewidth}t|p{0.165\linewidth}t|}
\hline
...
\hline
Infection {(day)}
  & ${\muIR^{-1}=\Sexpr{myround(7/p1["gamma"],1)}}\fixed$
  & \eqref{model1:toR}
  & ${\muIR^{-1}=\Sexpr{myround(365/h2_fit$h2_params["gamma"],1)}}\fixed $
  & \eqref{model2:mu_IR}
  & ${\muIR^{-1}=\Sexpr{myround(365/p3["gamma1"],1)}}\fixed $
  & \eqref{eq:model3:IR}
\\\hline
Latency {(day)}
  & ${\muEI^{-1}=\Sexpr{myround(7/p1["sigma"],1)}}\fixed$
  & \eqref{model1:EA}
  & ${\muEI^{-1}=\Sexpr{myround(365/h2_fit$h2_params["gammaE"],1)}}\fixed$
  & \eqref{model2:mu_EI}
  & \missing
  &
\\
...
\multirow{3}{*}{Seasonality}
  & $\transmission_{1:6}=(
      \Sexpr{myround(p1["beta1"],1)},\allowbreak
      \Sexpr{myround(p1["beta2"],1)},\allowbreak
      \Sexpr{myround(p1["beta3"],1)},\allowbreak
      \Sexpr{myround(p1["beta4"],1)},\allowbreak
      \Sexpr{myround(p1["beta5"],1)},\allowbreak
      \Sexpr{myround(p1["beta6"],1)})$
  & \eqref{model1:beta}
  & \multirow{2}{*}{${\seasAmplitude=0.4}\fixed$}
  & \multirow{2}{*}{\eqref{model2:lambda}}
  & \multirow{2}{*}{$\seasAmplitude = \Sexpr{myround(p3s["lambdaR"],2)}$}
  & \multirow{2}{*}{\eqref{eq:model3:water}}
  \\
  & $\transmissionTrend = \Sexpr{myround(p1["betat"],2)}^*$
  & \eqref{model1:betat}
  & \multirow{-2}{*}{$\phaseParm = \Sexpr{myround(h2_fit$h2_params['phase'], 2)}^*$}
  & \multirow{-2}{*}{\eqref{model2:lambda}}
  & \multirow{-2}{*}{$\rainfallExponent = \Sexpr{signif(p3s["r"],3)}$}
  & \multirow{-2}{*}{\eqref{eq:model3:water}}
  \\\hline
  \multirow{3}{*}{Immunity {(yr)}}
  & \multirow{3}{*}{${\muRS^{-1}=\Sexpr{myround(1/p1["alpha"]/52,1)}}\fixed$}
  & \multirow{3}{*}{\eqref{model1:RS}}
  & $\muRS^{-1}= \Sexpr{signif(1/h2_fit$h2_params["sigma"],2)}$
  & \eqref{model2:RS}
  & \multirow{3}{*}{${\muRS^{-1}=\Sexpr{myround(1/p3['rho1'],1)}}\fixed$}
  & \multirow{3}{*}{\eqref{eq:model3:RRnext}}
\\
& & & ${\omega_1^{-1} = \Sexpr{myround(1/h2_fit$h2_params["Omega1"],1)}}\fixed$ & \eqref{model2:omega1} & &\\
& & & ${\omega_2^{-1} = \Sexpr{myround(1/h2_fit$h2_params["Omega2"],1)}}\fixed$ & \eqref{model2:omega2} & &\\
\hline
\multirow{2}{*}{Birth/death {(yr\textsuperscript{-1})}}
  & ${\muBirth =  10^{-2}\times \Sexpr{myround(100 * 52 * p1["mu"],2)}}\fixed$
    %
  & \eqref{model1:death}
  & \multirow{2}{*}{\missing}
  &
  & ${\muDeath = 10^{-2}\times\Sexpr{myround(100 * p3["mu1"],2)}}\fixed$
  & \multirow{2}{*}{\eqref{eq:model3:IS}}
\\
& ${\muDeath = 10^{-3}\times\Sexpr{myround(1000*52*p1["delta"],1)}}\fixed$
& \eqref{model1:death}
&
&
& ${\choleraDeath = \Sexpr{myround(p3["alpha1"],2)}}\fixed$
&
\\\hline
Sympt. frac.
  & ${\symptomFrac_z(t)=c\vaccineEfficacy^*(t-\tau_d)}\fixed$
  & (\ref{model1:EI}-\ref{model1:EA})
  & ${\symptomFrac=\Sexpr{myround(h2_fit$h2_params["k"],1)}}\fixed$
  & \eqref{model2:mu_EI}
  & ${\symptomFrac=\Sexpr{myround(p3["sigma1"],2)}}\fixed$
  & \eqref{eq:model3:SA}
\\\hline
Asympt.
...
&
&
& ${\asymptomRelativeShed = \Sexpr{1e-7}}\fixed$
& \eqref{model2:to_W}
...
\multirow{4}{*}{Human to human}
  ...
  & \multirow{4}{*}{$\transmission=$\Sexpr{signif(h2_fit$h2_params["Beta"],3)} \allowbreak {\tiny yr\textsuperscript{-1}}}
  & \multirow{4}{*}{\eqref{model2:lambda}}
  & $\transmission_{1:10}=(
  \Sexpr{myround(p3u["foi_add",1]*1e6,1)}, \allowbreak
	\Sexpr{myround(p3u["foi_add",2]*1e6,1)}, \allowbreak
	\Sexpr{myround(p3u["foi_add",3]*1e6,1)}, \allowbreak
  \Sexpr{myround(p3u["foi_add",4]*1e6,1)}, \allowbreak
	\Sexpr{myround(p3u["foi_add",5]*1e6,1)}, \allowbreak
	\Sexpr{myround(p3u["foi_add",6]*1e6,1)}, \allowbreak
	\Sexpr{myround(p3u["foi_add",7]*1e6,1)}, \allowbreak
	\Sexpr{myround(p3u["foi_add",8]*1e6,1)}, \allowbreak
	\Sexpr{myround(p3u["foi_add",9]*1e6,1)}, \allowbreak
	\Sexpr{myround(p3u["foi_add",10]*1e6,1)}
	) \allowbreak \times 10^{-6}$ {yr\textsuperscript{-1}}
...
\multirow{5}{*}{Water to human}
  & \multirow{5}{*}{\missing}
  &
  & \multirow{3}{*}{${\Wsat = \Sexpr{signif(h2_fit$h2_params["Sat"],2)}}\fixed$}
  & \multirow{3}{*}{\eqref{model2:lambda}}
  & $\Wbeta{_{1:10}}= (
        \Sexpr{myround(p3u["betaB",1],1)}, \allowbreak
        \Sexpr{myround(p3u["betaB",2],1)}, \allowbreak
        \Sexpr{myround(p3u["betaB",3],1)}, \allowbreak
        \Sexpr{myround(p3u["betaB",4],1)}, \allowbreak
        \Sexpr{myround(p3u["betaB",5],1)}, \allowbreak
        \Sexpr{myround(p3u["betaB",6],1)}, \allowbreak
        \Sexpr{myround(p3u["betaB",7],1)}, \allowbreak
        \Sexpr{myround(p3u["betaB",8],1)}, \allowbreak
        \Sexpr{myround(p3u["betaB",9],1)}, \allowbreak
        \Sexpr{myround(p3u["betaB",10],1)})$ {yr\textsuperscript{-1}}
  & \multirow{4}{*}{\eqref{eq:model3:foi}}
\\
& & & \multirow{-3}{*}{$\beta_W= \Sexpr{signif(h2_fit$h2_params["BetaW"],3)}$ {yr\textsuperscript{-1}}} & \multirow{-3}{*}{\eqref{model2:lambda}} & &
\\
\hline
Human to water
  & \missing
  &
  & $\Wshed = $ \Sexpr{signif(h2_fit$h2_params["Mu"]/52,3)} {wk\textsuperscript{-1}}
  & \eqref{model2:to_W}
  & $\Wshed= \Sexpr{signif(p3s["thetaI"]/52,3)}$ {\tiny	$\frac{\mathrm{km^2}}{\mathrm{wk}}$}
  & \eqref{eq:model3:water}
\\
\raggedright Water survival (wk)
  & \missing
  &
  & ${\Wremoval^{-1} = \Sexpr{signif(52/h2_fit$h2_params["Delta"],3)}}\fixed$
  & \eqref{model2:from_W}
...
\\
\hline
\raggedright Process noise {(wk\textsuperscript{1/2})}
  & \raggedright $\sigmaProc = (\Sexpr{myround(p1["sig_sq_epi"],2)},
  \Sexpr{myround(p1["sig_sq_end"],2)})^*$
  & \multirow{2}{*}{\eqref{model1:lambda}}
  & \multirow{2}{*}{\missing}
  &
  & \multirow{2}{*}{$\sigmaProc= \Sexpr{myround(p3s["std_W"] * sqrt(52),3)}$}
  & \multirow{2}{*}{\eqref{eq:model3:SA}}
\\\hline
Reporting rate
  & $\reportRate=\Sexpr{myround(p1["rho"], 3)}$
  & \eqref{model1:meas}
  & ${\reportRate=\Sexpr{myround(h2_fit$h2_params["Rho"],2)}}\fixed$
  & \eqref{model2:meas}
  & $\reportRate=\Sexpr{myround(p3s["epsilon"],2)}$
  & \eqref{model2:meas}
\\\hline
\raggedright Observation variance
  & \raggedright $\obsOverdispersion = (\Sexpr{myround(p1["tau_epi"],1)}, \Sexpr{myround(p1["tau_end"],1)})$
  & \multirow{2}{*}{\eqref{model1:meas}}
  & \multirow{2}{*}{$\obsOverdispersion = \Sexpr{myround(h2_fit$h2_params['v'], 1)}$}
  & \multirow{2}{*}{\eqref{model2:meas}}
  & \multirow{2}{*}{$\obsOverdispersion = \Sexpr{myround(p3s["k"],1)}$}
  & \multirow{2}{*}{\eqref{model3:meas}}
\\
\hline
\multirow{2}{*}{Initial Values} & $\Iinit = \Sexpr{round(prod(p1[c('pop_0', 'I_0')]))}$
  & 
  & \multirow{2}{*}{\missing}
  &
  & \multirow{2}{*}{$\Iinit^{3,4} = (
        \Sexpr{round(prod(p3[c('H3', 'Iinit3')]))}, \Sexpr{round(prod(p3[c('H4', 'Iinit4')]))})^*$}
  &  \multirow{2}{*}{\missing}
  \\
  & $E_{0,0} = \Sexpr{round(prod(p1[c('pop_0', 'E_0')]))}$ & & \multicolumn{1}{c}{}
  &
  &
  &
\\\hline
\multirow{3}{*}{Hurricane}
  ...
  & \multirow{4}{*}{\missing}
  &
  & \raggedright $\Whur{3, 9} = (\Sexpr{myround(p3['aHur3'], 2)}, \Sexpr{myround(p3['aHur9'], 2)})^*$
...
\\
 \multirow{-1}{*}{Parameters} &
  ...
  &
  &
  & \raggedright $\hHur{3, 9} = (\Sexpr{myround(p3['hHur3'], 2)}, \Sexpr{myround(p3['hHur9'], 2)})^*$
  & \multirow{2}{*}{\eqref{eq:model3:foi}}
\\\hline
\end{tabular}
\caption[Parameter estimates of the cholera models.]{\label{tab:params}
Parameter estimates of the cholera models. References to the relevant equation are given in parentheses.
Parameters that were fixed and not calibrated using the data are indicated with $\fixed$; all fixed parameters values were chosen to match the fixed parameter values of \citet{lee20}.
Parameters that were added during our re-analysis and were not considered by Lee et al. are indicated with $^*$.
Confidence intervals for model parameters are given in Appendix~\ref{sec:appendix_haiti_ci}.
Translations back into the notation of \citet{lee20} are given in Table~\ref{tab:translate}.
}
% \begin{flushleft}
% \end{flushleft}
% \end{adjustwidth}
...
\begin{table}
  ...
  \end{tabular}
  \end{center}
  \caption{\label{tab:translate}Parameter translations between our common notation and notation used by Lee et al (2020).}
\end{table}

Each model describes the cholera dynamics as a partially observed Markov process (POMP).
Here, we use the same notation and definitions for POMP models provided in Chapter~\ref{chpt:intro}.
The latent state state $\bm{X}(t)$ for each model is modeled as a continuous time process, and consists of individuals labeled as susceptible (S), infected (I), asymptomatically infected (A), vaccinated (V), and recovered (R), with various sub-divisions sometimes considered.
...
% with a latent state vector $\bm{X}(t)$ for each continuous time point $t$.
...
% Together, with the density for the initial value of the latent state $\bm{X}_{0} = \bm{X}(t_0)$, each model defines a joint density $f_{\bm{X}_{0:N}, \bm{Y}_{1:N}}\big(\bm{x}_{0:N}, \bm{y}_{1:N};\paramVec\big)$, where $\paramVec$ is a parameter vector that indexes the model.
...
% Using the conditional independence of $\bm{Y}_{1:N}$ given $\bm{X}_{0:N}$ and the Markov property of $\bm{X}_{0:N}$, the joint density can be re-factored into the useful form given in Eq.~\myeqref{eq:jointLik}:
...
% The latent state vector $\bm{X}(t)$ for each model is modeled as a continuous time process, and consists of individuals labeled as susceptible (S), infected (I), asymptomatically infected (A), vaccinated (V), and recovered (R), with various sub-divisions sometimes considered.
% The observable random vector $\bm{Y}_{1:N}$ represents the random vector of cholera incidence data for each model;
% Models~2 and~3 have metapopulation structure, meaning that each individual is a member of a spatial unit, denoted by a subscript $u\in \seq{1}{U}$, in which case we denote the observed data for each unit using $\bm{y}_{1:N}^* = y_{1:N,1:U}^*$.
% Here, the spatial units are the $U=10$ Haitian administrative d\'{e}partements (henceforth anglicized as departments).
...
Additional details about the numeric implementation of these models are provided in Appendix~\ref{sec:appendix_haiti_details}.
...
For this reason, we provide flow chart diagrams for Models~1--3 in Figures~\ref{fig:flow_diagram}--\ref{fig:flow_diagram3}.
...
Here, $\vaccCounter=0$ corresponds to unvaccinated individuals, and $\vaccCounter \in \seq{1}{\vaccClass}$ describes hypothetical vaccination programs. Each program $\vaccCounter$ indexes differences in both the number of doses administered (one versus two doses per individual) and the round of vaccine administration, separating individuals into compartments with distinct dynamics based on vaccination status.
...
\end{equation}
where $\bar{\transmission} = 1$ (wk)\textsuperscript{-1} is a dimensionality constant.
...
\citet{lee20} included process noise in Model~3 but not in Model~1, i.e., they fixed $\sigmaProc^2 = 0$.
...
where $\vaccineEfficacy^*(t)$ is efficacy at time $t$ since vaccination for adults, a step-function represented in Table~S4 of \citet{lee20}, and $c=\big(1-(1-0.4688)\times 0.11\big)$ is a correction to allow for reduced efficacy in the 11\% of the population aged under 5 years.
...
These parameters are estimated using the observed data, with parameter estimates provided in Table~\ref{tab:params}. 

The measurement model describes reported cholera cases $Y_n$ at the nationally aggregated level at each week $n$ come from a negative binomial distribution, where only a fraction $\reportRate$ of new weekly cases are reported.
Because vaccinated individuals are treated as asymptomatic, all reported cases are modeled as transitions from the exposed to the infected compartment, as described in Eq.~\myeqref{model1:meas}. 
\begin{equation}
  \label{model1:meas}
...
The \citet{lee20} implementation of Model~2 fixes $\phaseParm = 0$.
...
\begin{eqnarray}
\label{model2:mu_SE}
\mu_{S_{u\vaccCounter}E_{u\vaccCounter}} &=& (1 - \vaccineEfficacy_\vaccCounter) \, \lambda_u(t),
\\
\label{model2:mu_EI}
...
$D_{u\varv}$ is a distance measure estimating average road distance between randomly chosen members of each population, and $v_{\mathrm{rate}}= 10^{-12} \,\mbox{km$^2$yr$^{-1}$}$ was fixed at the value used in \citet{lee20}.
...
In Eq.~\myeqref{model2:mu_SE}, $\vaccineEfficacy_\vaccCounter$ denotes the vaccine efficacy for each vaccination campaign $\vaccCounter \in \vaccClass$, with $\vaccineEfficacy_0 = 0$, $\vaccineEfficacy_1 = 0.429 \childReduce$, $\vaccineEfficacy_2 = 0.519 \childReduce$, $\vaccineEfficacy_3 = 0.429$, and $\vaccineEfficacy_4 = 0.519$
Here, $\childReduce = 0.4688$ represents the reduced efficacy of the vaccination for children under the age of five years, and the values $0.429$ and $0.519$ are the median effectiveness of one and two doses over their effective period respectively, according to Table S4 in the supplement material of \citet{lee20}.
...
Reported cases are described using a log-normal distribution, with the log-scale mean equal to the reporting rate $\reportRate$ times the number of newly infected individuals, following Eq.~\myeqref{model2:meas}.
...
This measurement model differs from that used by \citet{lee20}, who fit the model in two stages: epidemic and endemic phases, accounting for symptomatic infections in the endemic phase and both symptomatic and asymptomatic infections in the endemic period. 
...
\end{equation}
where $\tHur$ is the time Hurricane Matthew struck Haiti \citep{ferreirai16}, and $1_{(A)}$ is the indicator function for event $A$.
In \citet{lee20}, $\Whur{_u}$ and $\hHur{u}$ were set to zero for all $u$;
...
This is the same approach that was taken by \citet{lee20}, who used the value $t_0 = \text{2014-02-22}$; this choice of $t_0$ results in modeling roughly only $60\%$ of the available data, some of which is later discarded for alternative reasons.
...
\\
\label{eq:model3:B0}
W_{u}\big(t_0\big) &=& \big[1 + \seasAmplitude \tilde{J}^r \big] \mathrm{Den}_u \, \Wshed \big[ I_{u}(t_0)+ \asymptomRelativeShed A_{u_0}(t) \big]/\Wshed.
\end{eqnarray}
...
Finally, reported cholera cases are assumed to stem from individuals who develop symptoms and seek healthcare.
...
This measurement model is a minor change from that used by \citet{lee20}, which allowed for a change in the reporting rate on January 1st, 2018.
Their estimated reporting rate value before and after this period were $0.97$ and $0.097$, respectively.
...
To avoid this extreme possibility, we do not allow a change in reporting rate when fitting the model.

\subsection{Model Fitting}\label{sec:model_fitting}

Each of the three models considered in this study describes cholera dynamics using a POMP model, with the understanding that the deterministic Model~2 is a special case of a Markov processes solving a stochastic differential equation in the limit as the noise parameter goes to zero.
...
Therefore, the choice of $\paramVec$ used to make inference about the system can greatly affect model-based conclusions \citep{saltelli20}.
...
Calibrating model parameters to observed data does not guarantee that the resulting model successfully approximates real-world mechanisms, since the model description of the dynamic system may be incorrect and does not change as the model is calibrated to data.
...
In this chapter we follow \citet{lee20} by calibrating the parameters of each of our models using maximum likelihood, as described in Eq.~\myeqref{eq:likedef}.
...
The main alternative to maximum likelihood estimation is Bayesian inference via Markov chain Monte Carlo, used to analyze the Haiti cholera epidemic by \citet{andrews11,pasetto18,rinaldo12,lewnard16,trevisin22,sallah17,azman12,kuhn14,mari15,gatto12}.
...
\multirow{2}{*}{Log-likelihood} &
  ...
  ($\Sexpr{myround(unname(lee20_replication['leeFit_ll']), 1)}$) &
...
Number of &
 ...
 ($\Sexpr{as.character(unname(lee20_replication['leeFit_n_params']))}$) &
...
\hline
\thead{Benchmark \\ AIC} &
  $\Sexpr{myround(-2 * benchmarks$m1 + 2 * 4, 1)}$ &
  $\Sexpr{myround(-2 * benchmarks$m2 + 2 * 40, 1)}$ &
  $\Sexpr{myround(-2 * benchmarks$m3 + 2 * 40, 1)}$
\\
\hline
\end{tabular}
\caption[AIC values for cholera models and benchmarks.]{\label{tab:likes}AIC values for cholera models and benchmarks. Values in parentheses are corresponding values obtained using the models of \citet{lee20}. \textsuperscript{1}The reported likelihood is an upper bound of the likelihood of the model in \citet{lee20}. \textsuperscript{2}In \citet{lee20}, Model~3 was fit to a subset of the data (March 2014 onward, excluding data from Ouest in 2015-2016).
On this subset, their model has a likelihood of $\Sexpr{myround(lee3_results$sub_ll_eval, 1)}$.
On this same subset, our model has a likelihood of $\Sexpr{myround(mod3_eval_res$subset_ll['est'], 1)}$.
Details of estimating the likelihood of the models used in \citet{lee20} are provided in Appendix~\ref{sec:appendix_haiti_lee20}.}
\end{table}

\subsubsection*{Calibrating Model~1 Parameters}\label{sec:fit1}

Model~1 is a highly nonlinear over-dispersed stochastic dynamic model, favoring a scientifically plausible description of cholera dynamics rather than one that is statistically convenient \citep{he10}.
...
Therefore in order to perform likelihood based inference on this model, we are restricted to use parameter estimation techniques that have the \emph{plug-and-play} property, which is that the fitting procedure only requires the ability to simulate the latent process rather than evaluating transition densities \citep{breto09,he10};
in the context of the notation and definitions employed in this chapter, this means that we only require the ability to simulate from $f_{\bm{X}_0}\big(\bm{x}_0;\paramVec\big)$ and $f_{\bm{X}_n|\bm{X}_{n-1}}\big(\bm{x}_{n}|\bm{x}_{n-1}; \paramVec\big)$ rather than needing to evaluate these densities.
Plug-and-play algorithms include Bayesian approaches like ABC and PMCMC \citep{toni09,andrieu10}, but here we use algorithms that enable maximum likelihood estimation.
To our knowledge, the only plug-and-play methods that have been effectively used to maximize the likelihood for arbitrary nonlinear POMP models are iterated filtering algorithms \citep{ionides15}, which modify the well-known \emph{particle filter} \citep{arulampalam02}.
...
Iterated filtering algorithms, such as IF2 \citep{ionides15}, extend the particle filter by performing a random walk for each parameter and particle;
...
We demonstrate this capability by proposing a log-linear trend $\transmissionTrend$ in transmission in Eq.~\myeqref{model1:beta}:
\begin{equation}
\label{model1:betat}
...
The proposal of a trend in transmission is a result of observing an apparent decrease in reported cholera infections from 2012-2019 in Figure~\ref{fig:CholeraData}.
While several factors may contribute to this decrease, one explanation is that case-area targeted interventions (CATIs), which included education sessions, increased monitoring, household decontamination, soap distribution, and water chlorination in infected areas \citep{rebaudet19CATI}, may have substantially reduced cholera transmission over time \citep{rebaudet21}.

We perform a statistical test to determine whether or not the data indicate the presence of a trend in transmissibility.
To do this, we perform a profile-likelihood search on the parameter $\transmissionTrend$ and obtain a $95\%$ confidence interval via a Monte Carlo Adjusted Profile (MCAP) \citep{ionides17}.
\citet{lee20} implemented Model~1 by fitting two distinct phases: an epidemic phase from October 2010 through March 2015, and an endemic phase from March 2015 onward.
...
The resulting 95\% confidence interval for $\transmissionTrend$ is $(\Sexpr{myround(beta_t_mcap_results$ci[1], digits = 3)}, \Sexpr{myround(beta_t_mcap_results$ci[2], digits = 3)})$, with the full results displayed in Figure~\ref{fig:betat}.
...
The maximum likelihood estimate for $\transmissionTrend$ corresponds to a \Sexpr{paste0(myround(100 - 100*exp(beta_t_mcap_results$mle * 430/215), 1), '\\%')} reduction to the transmission rate over the course of the outbreak, with a $95\%$ confidence interval of (\Sexpr{paste0(rev(myround(100 - 100*exp(beta_t_mcap_results$ci * 430/215), 1)), collapse = ', ', sep = '\\%')}) for the overall reduction in transmission.
...
The inclusion of a trend in transmission rate demonstrates a class of model variation that can be highly beneficial to consider: the model variation has a plausible scientific justification, and is easily testable using likelihood based methods.
...
ggplot() +
  ...
  geom_line(data = beta_t_mcap_results$fit, aes(x = parameter, y = smoothed), col = 'blue') +
  # geom_line(data = beta_t_mcap_results$fit, aes(x = parameter, y = quadratic), col = 'red') +
  geom_vline(xintercept = beta_t_mcap_results$ci[1], linetype = 'dashed') +
  geom_vline(xintercept = beta_t_mcap_results$ci[2], linetype = 'dashed') +
  geom_vline(xintercept = beta_t_mcap_results$mle, col = 'blue') +
  labs(x = "Log-Linear Trend in Transmission", y = 'Log Likelihood') +
  theme(axis.text = element_text(size = 8),
        axis.title = element_text(size = 10))
@
\caption[Confidence interval for linear trend in transmission rate.]{\label{fig:betat}
Confidence interval for linear trend in transmission rate. The confidence interval is calculated using Monte Carlo adjusted profile (MCAP) of $\transmissionTrend$ for Model~1.
...
<<clean Up of Betat data, echo=FALSE, message=FALSE, include=FALSE, warning=FALSE>>=
suppressWarnings(
  rm(h1_prof_res, beta_t_mcap_results)
)
gc()
@
...
The statistical evidence of a trend in transmission rate in this model could be explained by any trending variable (such as hygiene improvements, or changes in population behavior), resulting in confounding from collinear covariates. Alternatively, it is possible that the negative trend observed in the incidence data could be attributed to a decreasing reporting rate rather than decreasing transmission rate. This could be formally tested by comparing models with either trend specification. We did not do this because evidence suggests that reporting rate was maintained or increased (Figure~1 of \citet{rebaudet21}). We instead argue that a decreasing transmission rate is a plausible way to explain the decrease in cases over time, as there is alternative evidence that supports this model \citep{rebaudet19CATI,rebaudet21,michel19}. It is not practical to test all remotely plausible model variations, yet a strongly supported conclusion should avoid ruling out untested hypotheses. The robust statistical conclusion for our analysis is that a model which allows for change fits better than one which does not, and a trend in transmission is a plausible way to do this.

We implemented Model~1 using the \code{pomp} package \citep{king16}, relying heavily on the source code provided by \citet{lee20}.
...
The large increase in the log-likelihood for Model~1 (Table~\ref{tab:likes}) can primarily be attributed to increasing the computational effort used to calibrate the model.
...
sims <- simulate(
  h1, nsim = switch(RUN_LEVEL, 50, 100, 500),
  format = 'data.frame', seed = 3448931
)
...
@
\caption[Simulations from Model~1 compared to reported cholera cases.]{\label{fig:mod1fit}
Simulations from Model~1 compared to reported cholera cases. The black curve is observed data, the blue curve is median of 500 simulations from initial conditions using estimated parameters, and the vertical dashed line represents break-point when parameters are refit.
}
\end{figure}
...
The use of deterministic compartment models have a long history in the field of infectious disease epidemiology \citep{kermack1927,brauer2017,giordano20},
and can be justified by asymptotic considerations in a large-population limit \citep{dadlani2020,ndii17}.
Because the process model of Model~2 is deterministic, maximum likelihood estimation reduces to a least squares calculation when combined with a Gaussian measurement model (Eq.~\myeqref{model2:meas}).
\citet{lee20} fit two versions of Model~2 based on a presupposed change in cholera transmission from a epidemic phase to endemic phase that occurred in March, 2014.
...
Here we chose to fit the model to transformed case counts rather than adding overdispersion to the measurement model with the goal of minimizing the changes to the model proposed by \citet{lee20}.

We implemented this model using the \code{spatPomp} \code{R} package \citep{asfaw24}.
The model was then fit using the subplex algorithm \citep{king2020Subplex}.
A comparison of the trajectory of the fitted model to the data is given in Figure~\ref{fig:mod2Traj}.
...
mod2_VaccScenarios <- readRDS(paste0(root, 'data/model2/VaccinationScenarios.rds'))
@
...
ggplot() +
  ...
  geom_ribbon(
    ...
    aes(
      x = date,
      ymin = (Ctotal + 1) * exp(-1.96*h2_fit$h2_params['v']),
      ymax = (Ctotal + 1) * exp(1.96*h2_fit$h2_params['v'])
    ), alpha = 0.5
  ) +
  geom_line(data = true_agg_cases, aes(x = date, y = ReportedAll + 1)) +
  geom_line(
    data = filter(mod2_VaccScenarios$all_trajs, year >= max(h2_traj$year)),
    aes(x = as.Date(date), y = ReportedCases + 1, color = scenario)
  ) +
  geom_ribbon(
    data = filter(mod2_VaccScenarios$all_trajs, year >= max(h2_traj$year)) %>% filter(scenario == 'V0'),
    aes(
      x = as.Date(date),
      ymin = pmax((ReportedCases + 1) * exp(-1.96*h2_fit$h2_params['v']), 1),
      ymax = pmax((ReportedCases + 1) * exp(1.96*h2_fit$h2_params['v']), 1)
...
@
\caption[Simulated trajectory of cholera incidence using Model~2.]{\label{fig:mod2Traj}
Simulated trajectory of cholera incidence using Model~2.
The black line shows the nationally aggregated weekly cholera incidence data.
The blue curve from 2012-2019 is the trajectory of the calibrated model.
Projections under the various vaccination scenarios, which are discussed in detail in the  {\bf Forecasts} subsection are also included. The gray ribbons represent a $95\%$ interval obtained from the log-normal measurement model. To avoid over-plotting, measurement variance is only plotted for the V0 vaccination scenario.}
\end{figure}
...
However, a legitimate preference for simplicity can support a decision to consider nationally aggregated models \citep{saltelli20,green15}.

In our literature review, 17 articles considered dynamic models that incorporate spatial heterogeneity \citep{lee20,tuite11,pasetto18,fitzgibbon20,eisenberg13,rinaldo12,chao11,abrams13,trevisin22,sallah17,collins14,kelly16,azman12,leung22,kuhn14,mari15,gatto12}. All but four \citep{lee20,pasetto18,sallah17,azman12} of these studies used deterministic dynamic models: this greatly simplifies the process of calibrating model parameters to incidence data, though deterministic models can struggle to describe complex stochastic dynamics. The model in \citep{pasetto18} was fit using an Ensemble Kalman Filter (EnKF) \citep{evensen09}; though EnKF scales favorably with the number of spatial units, it relies on linearization of latent states which can be problematic for highly nonlinear systems \citep{evensen22,ionides21}. Alternative approaches used to fit stochastic models included making additional simplifying assumptions to aid in the fitting process \citep{lee20}, and using MCMC algorithms \citep{sallah17,azman12} which require specific structures in the latent dynamics, making these algorithms non plug-and-play. In this subsection, we present how the recently developed iterated block particle filter (IBPF) algorithm \citep{ning23,ionides24} can be used to fit a spatially explicit stochastic dynamic model to incidence data.
...
To avoid the approximation error present in high-dimensional models, \citet{lee20} simplified the problem of estimating the parameters of Model~3 by creating an approximate version of the model where the units are independent given the observed data.
...
Because the simplified, spatially-decoupled version of Model~3 implemented in \citet{lee20} relies on the observed cholera cases, the calibrated model cannot readily be used to obtain forecasts.
Therefore, in order to obtain model forecasts, \citet{lee20} used the parameters estimates from the spatially-decoupled approximation of Model~3 to obtain forecasts using the fully coupled version of the model.
...
These two concerns may explain the unrealistic forecasts and low likelihood of Model~3 in \citet{lee20} (Table~\ref{tab:likes}).

At the time \citet{lee20} conducted their study, there was no known algorithm that could readily be used to maximize the likelihood of an arbitrary meta-population POMP model with coupled spatial dynamics, which justifies the spatial decoupling approximation that was used to calibrate model parameters.
For our analysis, we calibrate the parameters of the spatially coupled version of Model~3 using the IBPF algorithm \citep{ionides24}.
This algorithm extends the work of Ning and Ionides \citep{ning23}, who provided theoretic justification for the version of the algorithm that only estimates unit-specific parameters.
...
Simulations from the fitted model are displayed in Figure~\ref{fig:h3spatsims}.
...
h3_quants <- readRDS(paste0(root, 'data/model3/h3Sims.rds'))
@
...
@
\caption[Simulations from Model~3 compared to reported cholera cases.]{\label{fig:h3spatsims}
Simulations from Model~3 compared to reported cholera cases. Simulations were obtained using the initial conditions of Model~3 calibrated to data.
...
The goal of parameter calibration---whether done using Bayesian or frequentist methods---is to find the best description of the observed data in the context of the model.
...
To make progress, while accepting proper limitations, one must bear in mind the much-quoted observation of George Box \citep{box79} that ``all models are wrong but some are useful.''
Beyond being good practical advice for applied statistics, this assertion is relevant for the philosophical justification of statistical inference as severe testing \citep{mayo18}. % \citep[][Sec.~4.8]{mayo18}.
...
The study by \citet{lee20} provides an example of this: their models and parameter estimates resulted in simulations that visually resembled the observed data, yet resulted in model likelihoods that were considerably smaller than likelihoods that can be achieved (see Table~\ref{tab:likes}).
...
These comparisons, which provide insight into how each model performs relative to the others, are quite common \citep{rinaldo12,sallah17}.
...
Examples include linear regression, auto regressive moving average (ARMA) time series models, or even independent and identically distributed measurements.
...
In the case where the case counts are large, an alternative benchmark recommended by \citet{he10} is a log-linear Gaussian ARMA model;
...
Models that are fit to the same datasets can be directly compared using AIC values, making it a useful tool to compare to benchmark models. 
Though Models~2 and 3 are both fit to department level incidence reports, their AIC values are not directly comparable due to the way Model~3 initializes latent states.
...
Additional techniques include the inspection of conditional log-likelihoods of each observation given the previous observations in order to understand how well the model describes each data point (Appendix~\ref{sec:appendix_haiti_mod3Cal}).
Other tools include plotting the effective sample size of each observation \citep{liu01}; plotting the values of the hidden states from simulations (Appendix~\ref{sec:appendix_haiti_mod3Cal});
and comparing summary statistics of the observed data to simulations from the model \citep{wood10,king15}.
...
On the other hand, comparisons may generate unexpected results that have the potential to spark new scientific knowledge \citep{ganusov16}.
...
Figure~\ref{fig:h1SeasRain} shows that the estimated seasonal transmission rate $\transmission$ mimics the rainfall dynamics in Haiti, despite Model~1 not having access to rainfall data.
This is consistent with previous studies that incorporated rainfall as an important part of their mechanistic model or otherwise argue that rainfall is an important driver of cholera dynamics in Haiti \citep{hulland19,kirpich17,lee20,moise20,pasetto18,kirpich16,eisenberg13,rinaldo12,mavian20}.
The estimated seasonality also features an increased transmission rate during the fall, which was noticed at an earlier stage of the epidemic \citep{rinaldo12}. The high transmission rate in the fall may be a result of the increase transmission that occurred in the fall of 2016, when hurricane Matthew struck Haiti \citep{ferreirai16}.
...
# Create a figure showing annual rainfall, scaled from 0-1. This is top figure
gg_rain <- df_rain %>%
  ...
  theme(  # Remove elements that are un-unused because this is top figure
    axis.title.x = element_blank(),
...
@
\caption[Seasonality of Model 1 transmission rates and weekly rainfall.]{\label{fig:h1SeasRain}
Seasonality of Model 1 transmission rates and weekly rainfall.
...
In epidemiological studies, for example, quantitative understanding of individual-level processes may not perfectly match model parameters that were fit to population-level case counts, even when the model provides a strong statistical fit \citep{he10}.
...
Under the framework of Model~2, the best estimate for this parameter is $\Sexpr{signif(1/h2_fit$h2_params["sigma"],3)}$ yr, suggesting that individuals have permanent immunity to cholera once infected.
Rather than interpreting this as scientific evidence that individuals have permanent immunity from cholera, this result suggests that Model~2 favors a regime where reinfection events are a negligible part of the dynamics.
The depletion of susceptible individuals may be attributed to confounding mechanisms---such as localized vaccination programs and non-pharmaceutical interventions that reduce cholera transmission \citep{trevisin22, rebaudet21}---that were not accounted for in the model.
...
For example, reconstructed latent variables (such as numbers of susceptible and recovered individuals) should make sense in the context of alternative measurements of these variables \citep{grad12}.
...
In Appendix~\ref{sec:appendix_haiti_mod3Cal}, we explore in more detail the process of model fitting and diagnostics for Model~3. Here we demonstrate that the model outperforms its benchmark model on the aggregate scale.
However, when focusing on the spatial units with the highest incidence of cholera, Model~3 performs roughly the same as a simple benchmark. By comparing simulations from the fitted model to the filtering distribution, we see that the reconstructed latent states of the model favor higher levels of cholera transmission than what is typically observed in the incidence data.
These results hint at the possibility of model mispecification, and warrant a degree of caution in interpreting the model's outputs.
...
In contrast, for non-deterministic Models~1 and 3, we obtain forecasts by simulating future values using latent states that are harmonious with the most recent data. This is done by simulating forward from latent states drawn at the last observation time ($t_N$) from the filtering distribution $f_{\bm{X}_N|\bm{Y}_{1:N}}(\bm{x}_{N} | \bm{y}^*_{1:N} ; \hat\paramVec)$. The decision to obtain model forecasts from initial conditions partially explains the unsuccessful forecasts of \citet{lee20}. Table~S7 in their supplement material, which contains results that were not discussed in their main article, shows that the subset of their simulations with zero cholera cases from 2019-2020 also correspond with its disappearance until 2022. These results support our argument that forecasts should be made by ensuring the starting point for the forecast is consistent with available data.

Uncertainty in just a single parameter can lead to drastically different forecasts \citep{saltelli20}.
...
For frequentist inference, one possible approach is obtaining model forecasts from various values of $\paramVec$, where the values of $\paramVec$ are sampled proportionally according to their corresponding likelihoods \citep{king15}. 
This approach is described in more detail in Appendix~\ref{sec:appendix_haiti_uncertain}.
...
In this analysis, we do not construct forecasts accounting for parameter uncertainty as our focus is on the estimation and diagnosis of mechanistic models, rather than providing forecasts intended to influence policy.
Furthermore, we use the projections from a single point estimate to highlight the deficiency of deterministic models that the only variability in model projections is a result of parameter and measurement uncertainty, which can lead to over-confidence in forecasts \citep{king15}.

The primary forecasting goal of \citet{lee20} was to investigate the potential consequences of vaccination interventions on a system to inform policy.
...
h1_S0_results <- readRDS(paste0(root, 'data/model1/S0_sims.rds'))
h1_S2_results <- readRDS(paste0(root, 'data/model1/S2_sims.rds'))
h1_S4_results <- readRDS(paste0(root, 'data/model1/S4_sims.rds'))
h1_S3_results <- readRDS(paste0(root, 'data/model1/S3_sims.rds'))
h1_S1_results <- readRDS(paste0(root, 'data/model1/S1_sims.rds'))
@

<<Simulate Model 3 Vacc Scenarios, message=FALSE, include=FALSE>>=
NP_BPF <- switch(RUN_LEVEL, 20, 200, 2000)
NSIM   <- switch(RUN_LEVEL, 5,  50, 1000)

# h3_bpf_end_states <- readRDS(paste0(root, 'h3_bpf_end_states.rds'))

mod3_V0_res <- readRDS(paste0(root, "data/model3/mod3_V0_res.rds"))
mod3_V1_res <- readRDS(paste0(root, "data/model3/mod3_V1_res.rds"))
mod3_V2_res <- readRDS(paste0(root, "data/model3/mod3_V2_res.rds"))
mod3_V3_res <- readRDS(paste0(root, "data/model3/mod3_V3_res.rds"))
mod3_V4_res <- readRDS(paste0(root, "data/model3/mod3_V4_res.rds"))
@
...
ggplot(tidyr::drop_na(all_prob_elims), aes(x = time, y = elim_prob, col = mod)) +
  ...
  scale_x_date(date_labels = "'%y", breaks = seq.Date(from = as.Date("2010-01-01"), as.Date("2030-01-01"), by = '2 years'))
@
\caption[Probability of cholera elimination under Models~1 and 3.]{\label{fig:elimProbs}
Probability of cholera elimination under Models~1 and 3. Elimination is defined as having zero cholera infectious for at least 52 consecutive weeks, based on 10 year simulations from the calibrated models.
Compare to Figure~3A of \citet{lee20}.}
\end{figure}
...
We therefore do not provide probability of elimination estimates under Model~2, but show trajectories under the various vaccination scenarios using this model (Figure~\ref{fig:mod2Traj}).

\section{Discussion}\label{sec:discussion}

The ongoing global COVID-19 pandemic has demonstrated how government policy may be affected by the inferences drawn from mathematical modeling \citep{saltelli20}.
However, the development of credible models---which are supported by data and can provide quantitative insights into a dynamic system---remains a challenging task.
In this chapter, we demonstrated opportunities available for raising the current standards of statistical inference for mathematical models of biological systems.

We presented methodology consistent with existing guidelines \citep{behrend20} but going beyond standard practice.
In particular, we showed the value of comparing the likelihood of fitted mechanistic models versus non-mechanistic benchmarks, a practice that has been previously advocated for \citep{he10} but was not done by any of the studies in our literature review.
These comparisons, along with other likelihood based diagnostics, help identify specific limitations of proposed models.
Diagnostic tools include likelihood profile methods, which help to assess parameter identifiability and enable the construction of confidence intervals for parameter estimates \citep{ionides17, simpson23}.
...
In our case study, employing deterministic dynamics in Model~2 was found to be an over-simplification by comparing model fit with benchmarks.
...
Model~3 is distinct in that it is both stochastic and has a meta-population structure, making it challenging to draw likelihood-based inferences. In this chapter, we demonstrated how this model class can be calibrated to incidence data using the innovative IBPF algorithm. One of only a few examples of fitting a nonlinear non-Gaussian meta-population model via maximum likelihood \citep{li23,ionides24}, this case study exemplifies the algorithm's potential benefits and provides an example for future researchers on a possible approach to fitting a high-dimensional non-linear model.
...
Unmodeled features of a dynamic system can lead to spurious or misleading parameter estimates if the features substantially impact observed data.
% A fitted model must compensate for these unaccounted features using its inherent flexibility.
In deterministic models, features that cannot be explained by measurement error must be accounted for by the choice of parameters.
% the choice of parameters is the primary source of flexibility to compensate for these unaccounted features.
For our case study, some of the parameter estimates for the deterministic Model~2 are implausible, such as the infinite immunity discussed above, and this may be explained by compensation for model misspecification.
...
Models~1 and 3 suggest the presence of extra-demographic stochasticity \citep{he10,stocks20,li23}, as evidenced by the confidence intervals for the corresponding parameter $\sigmaProc$ (Appendix~\ref{sec:appendix_haiti_ci}).

If forecasts are an important component of a modeling task, the forecasts should be consistent with the available data, particularly at the most recently available time points. In our case study, we did this by simulating forward from the filtering distribution, as this procedure conditions latent variables on the available data. This type of forecasting, however, is not directly available using a deterministic model, where future dynamics are fully determined by initial conditions and parameter values. This can result in over-confident model forecasts \citep{king15}. Despite their limitations, deterministic models can offer valuable insights into dynamic systems \citep{may04}. In \citet{lee20}, the forecasts from the deterministic Model~2 were qualitatively more consistent with the observed disappearance of cholera than the stochastic models. In our case study, we found improvements to Models~1 and 3 that resulted in improved forecasts for these models.

In our case study, we found that additional attention to statistical details could have resulted in an enhanced statistical fit to the observed incidence data. This would have improved the accuracy of the policy guidance resulting from the study.
We used the same data, models, and much of the same code used by \citet{lee20}, but we arrived at drastically different conclusions.
Specifically, each of the re-calibrated models predicted with moderate probability that cholera would disappear from Haiti.
...
Mechanistic models offer opportunities for understanding and controlling complex dynamic systems.
...
Researchers should ensure that intensive numerical calculations are adequately executed.
Using benchmarks and alternative model specifications to assess statistical goodness-of-fit should also should be common practice.
Once a model has been adequately calibrated to data, care is required to assess what causal conclusions can properly be inferred given the possibility of alternative explanations consistent with the data.
...
Cautionary warnings about the difficulties inherent in understanding complex systems \citep{saltelli20,ioannidis20,ganusov16} should motivate us to follow best practices in data analysis, rather than avoiding the challenge.
...
We provide a strong form of reproducibility, as well as extendability, by developing our analysis in the context of a software package, \code{haitipkg}, written in the R language \citep{r}.
Using a software package mechanism supports documentation, standardization and portability that promote extendability.
In the terminology of \citet{gentleman07}, the source code for this chapter is a {\it dynamic document} combining code chunks with text.
In addition to reproducing the content of this chapter, the code can be extended to examine alternative analysis to that presented.
