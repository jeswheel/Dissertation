...
@
\caption[Weekly cholera cases]{\label{fig:CholeraData}
...
References to the relevant equation are given in parentheses.
...
\begin{table}
  ...
  \end{tabular}
  \end{center}
  \caption[Parameter translations to original model definitions]{\label{tab:translate}Translations between our common notation and notation used by Lee et al (2020).}
\end{table}
...
\hline
\end{tabular}
\caption[AIC values for cholera models and benchmarks]{\label{tab:likes}AIC values for Models~1--3 and their benchmarks. Values in parentheses are corresponding values obtained using the models of \citet{lee20}. \textsuperscript{1}The reported likelihood is an upper bound of the likelihood of the model in \citet{lee20}. \textsuperscript{2}In \citet{lee20}, Model~3 was fit to a subset of the data (March 2014 onward, excluding data from Ouest in 2015-2016).
...
@
\caption[Trend in transmission rate confidence interval.]{\label{fig:betat}
...
The black curve is observed data, the blue curve is median of 500 simulations from initial conditions using estimated parameters, and the vertical dashed line represents break-point when parameters are refit.
}
\end{figure}
...
@
\caption[Simulated trajectory of Model~2]{\label{fig:mod2Traj}
