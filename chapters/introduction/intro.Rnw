...
Constructing a state space model involves proposing equations that describe the evolution of this unobservable process over time and how observations are made.
Common goals of state space modeling include making inferences about critical yet unobserved variables, incorporating real-world mechanisms into statistical models, and enabling likelihood based inference for models who that lack closed-form expressions of the likelihood function.
...
The term \emph{mechanistic} is reserved for models where the evolution of latent variables is dictated by equations intended to replicate real-world mechanisms, such as the transmission of infectious diseases among susceptible populations \citep{wheeler24}.
...
For convenience, we adopt the notation that for any integers $a < b$, $\seq{a}{b}$ is the vector $(a, a+1, \ldots, b-1, b)$.
...
The joint probability density or (mass function) of the observable and unobservable processes is assumed to exist, and can be written as $f_{\bm{X}_{0:N}, \bm{Y}_{1:N}}(\bm{x}_{0:N}, \bm{y}_{1:N}; \theta)$, where $\theta$ is a parameter vector $\theta \in \R^{d_\theta}$.
The likelihood is a function of $\theta$, defined by the density of $\bm{Y}_{1:N}$ evaluated at the observed data $\bm{y}^*_{1:N}$, given by
\begin{eqnarray}
\label{eq:likedef}
\mathcal{L}(\theta) = f_{\bm{Y}_{1:N}}\big(\bm{y}_{1:N}^*; \paramVec\big) = \int f_{\bm{X}_{0:N}, \bm{Y}_{1:N}}\big(\bm{x}_{0:N}, \bm{y}_{1:N}^*;\paramVec\big) d\bm{x}_{0:N}.
\end{eqnarray}
...
$$
f_{\bm{X}_{n} | \bm{X}_{1:n-1}}(\bm{x}_{n} | \bm{x}_{1:n-1}; \theta) = f_{\bm{X}_{n} | \bm{X}_{n-1}}(\bm{x}_{n} | \bm{x}_{n-1}; \theta),
$$
and
$$
f_{\bm{Y}_{n} | \bm{X}_{1:N}, \bm{Y}_{-n}}(\bm{y}_{n} | \bm{x}_{0:N}, \bm{y}_{-n}; \theta) = f_{\bm{Y}_{n} | \bm{X}_{n}}(\bm{y}_{n} | \bm{x}_{n}; \theta),
$$
...
\end{eqnarray}
This representation is useful as the entire model can be expressed in terms of three simple components, namely the \emph{initializer} $f_{\bm{X}_0}(\bm{x}_0; \theta)$, the \emph{process} or \emph{transition} model $f_{\bm{X}_{n} | \bm{X}_{n - 1}}(\bm{x}_n | \bm{x}_{n - 1}; \theta)$, and the \emph{measurement} model $f_{\bm{Y}_{n} | \bm{X}_n}(\bm{y}_n | \bm{x}_n ; \theta)$.
Each of these pieces can depend arbitrarily on observation time and the individual components of the parameter vector $\theta$.
...
popViewport()
@
\caption[Schematic of POMP model]{\label{fig:pompDiagram}A flow diagram representing an arbitrary POMP model.}
\end{figure}
