...
In other literature, the term \emph{mechanistic} is used for models that focus on the causality of input-output relationships \citep{baker18} and does not require that the model is a SSM or is otherwise used for time series analysis.
