...
When combined with a model relating the state to observable quantities, we obtain a partially observed Markov process (POMP) model, also known as a state space model or a hidden Markov model.
...
Contemporary applications of nonlinear mechanistic models for low-dimensional systems are abundant \citep{kramer24,he24,newman23}, yet examples of higher-dimensional panel equivalents remain sparse.
...
This algorithm, known as IF2, has seen extensive use for inference on low dimensional dynamic models, especially in epidemiological contexts \citep[e.g.,][]{pons-salort18,stocks18,subramanian21,fox22}.
...
\section{Iterated filtering for panel models}
...
Denoting the latent process for unit $u$ as $X_u(t)$ and its measurements as $Y_{u,n}$, a PanelPOMP can be expressed as a special case of a POMP model by writing $\mathbf{X}(t) = \{X_u(t)\}_{u = 1}^U$ and $\mathbf{Y}_{n} = \{Y_{u, n}\}_{u = 1}^U$, where we suppose the $U$ units are labeled $1,\dots, U$.
In principle, the observation times $t_{u, n} \in \mathcal{T}$ do not need to match for each unit, and therefore we assume $t_n \in \{t_{u, 0:N_u}\}_{u=1}^U$ such that $t_{n} \geq t_{m}$ if $n \geq m$, where $t_{u, 0:N_u}$ denotes the observation times for unit $u$.
The joint density of the PanelPOMP model can be written as
\begin{align}
...
\end{align}
where Eq.~\ref{eq:ppomp} arises from the assumption of dynamically independent units, and Eq.~\ref{eq:ppomp2} is a result of the Markov property of $X_{u}(t)$ and the conditional independence assumption on $Y_{u, n}$.

Our goal is to make inferences about the parameter $\theta$ by maximizing the likelihood function
\begin{align}
L(\theta;\mathbf{y}^*) &= \int f_{\mathbf{X}_{0:N}, \mathbf{Y}_{1:N}}(\mathbf{x}_{0:N}, \mathbf{y}^*_{1:N}; \theta)\, d\mathbf{x}_{0:N}. \label{eq:likelihood}
\end{align}
...
A special case of particular interest arises when $\theta = (\phi, \psi_{1:U})$, where $\phi \in \Theta_\phi \subset \mathbb{R}^{d_\phi}$ is a vector of parameters that are shared across each unit, and $\psi_{u} \in \Theta_\psi \subset \mathbb{R}^{d_u}$ are parameters that are specific to unit $u$, formally,
$$
...
The algorithmic complexity of both PIF and MPIF is $O(JMNU)$, where $M$ represents the number of iterations, $J$ represents the number of particles, $U$ represents the number of units in the panel, and $N$ is the mean of $\{N_1, \ldots, N_U\}$.
...
\begin{theorem}\label{theorem:pif}
  Consider a PanelPOMP model defined by Eq.~\ref{eq:ppomp}.
  Assume conditions \ref{assumption:1}--\ref{assumption:kernel} of Appendix~\ref{sec:assumptions} are satisfied, and write $\hat{\theta} = \argmax_{\theta} L(\theta;\mathbf{y}^*)$ from Eq.~\ref{eq:likelihood}.
...
If we denote $\pi_i(\theta)$ as the posterior distribution of the parameter vector $\theta$ after the $i$th Bayesian update, and $\mathbf{y}^* = \mathbf{y}_{1:N}^*$ as the observed data, we can express an iterated Bayesian update as the following:
\begin{align*}
\pi_1(\theta) &\propto f(\mathbf{y}^* ;\theta)\pi_0(\theta), \\
\pi_2(\theta) &\propto f(\mathbf{y}^* ;\theta)\pi_1(\theta) \propto f^2(\mathbf{y}^* ;\theta)\pi_0(\theta),\\
&\vdots \\
\pi_m(\theta) &\propto f^m(\mathbf{y}^* ;\theta)\pi_0(\theta).
\end{align*}
In this representation, $f(\mathbf{y}^*; \theta)$ is the likelihood function (Eq.~\ref{eq:likelihood}), and $\pi_0(\theta)$ is the original prior distribution for $\theta$.
...
% at each observation time and treating fixed parameters as latent states that perform a random walk at each time step.
...
% \citet{chen24} demonstrate that the output of the algorithm still concentrates on the MLE in this scenario.
...
For example, the particles representing the distribution of $\psi_U$ have been resampled $U-1$ times before encountering the data $y^*_U$, the only subset of data containing direct information about $\psi_U$.
This process can lead to significant particle depletion, particularly if $U$ is large (see Figure~\ref{fig:depletion}).
...
The use of independent priors is common practice in Bayesian statistics, but each full-iteration of Eq.~\ref{eq:PIFupdate} introduces parameter dependence after each $m$-iteration via the likelihood function.
% As the marginal likelihood at each sub-iteration $u$ only introduces dependence between $\phi$ and $\psi_u$, 
...
The marginalization step introduced by the MPIF algorithm, however, introduces a nonlinearity in the data cloning procedure.
...
As before, we assume there are $U \geq 1$ units, and the likelihood of each unit is defined by $L_u(\theta) = f_u(\mathbf{y}_u^*; \phi, \psi_u)$, and the likelihood of the entire model is $L(\theta) = \prod_{u = 1}^U L_{u}(\phi, \psi_u)$.
In what follows, we assume $\phi \in \R$ and $\psi_u \in \R$ for all $u \in \seq{1}{U}$ in order to ease the notation and analysis. 

\begin{theorem}
  Let $f_u(\mathbf{y}_u^*; \phi, \psi_u)$ be the density that corresponds to a Gaussian distribution with mean $(\phi^*, \psi_u^*)$ and precision $\Lambda_u^* \in \R^{2\times2}$. 
...
$$
% For small $U$, this condition is relatively weak, but becomes stronger as $U$ grows large. 
% It is in the large $U$ scenario, however, where MPIF demonstrably results in a significant improvement relative to PIF (See Figure~\ref{fig:gomp}), which does not have similar constraints (Theorem~\ref{theorem:pif}).
...
\begin{corollary}\label{corollary:perturbed}
  Consider the setup of Theorem~\ref{theorem:GG}. If the parameter perturbations are Gaussian with covariance matrix $\sigma^2_m \Sigma_0$ for some initial covariance matrix $\Sigma_0 \in \R^{(U+1) \times (U+1)}$ and sequence $\sigma^2_m = o(1/m)$, then the $m$th iteration of Eqs.~\ref{eq:margBayesPerturb} and \ref{eq:MPIFupdatePerturb} corresponds to a Gaussian distribution with mean $\mu'_m \in \R^{U+1}$ and covariance $\Sigma'_m \in \R^{U+1\times U+1}$. If $\theta^* = (\phi^*, \psi^*_1, \ldots, \psi^*_{U})$ denotes the MLE, then $\|\mu'_m - \theta^*\|_2 \rightarrow 0$ and $\|\Sigma'_m\|_2 \rightarrow 0$.
\end{corollary}

The convergence of the Eqs.~\ref{eq:margBayesPerturb}-\ref{eq:MPIFupdatePerturb} can be partially explained using a well known heuristic in Bayesian analysis: a more dispersed prior typically results in a posterior distribution that more closely resembles the likelihood function. 
...
In some cases, the marginalization step added to data cloning can result in faster convergence to the MLE if the marginalization increases the probability mass of the intermediate distributions in the direction of the MLE (Figure~\ref{fig:DC}). 
The primary benefit and motivation of the marginalization step, however, is in improving the particle representations of the intermediate parameter distributions. 
...
For our model, we suppose there are $Y_{u, n}$ are independent and identically distributed ($\iid$) from a normal $\normal[\psi_u, 1]$ distribution, and do not specify a latent process model as it is irrelevant for this model.
...
Theorem~\ref{theorem:GG} provides formal conditions where the effect of the added bias at each step still results in convergence to the MLE.
...
\noindent where $K_u$ is the carrying capacity for the population in unit $u$, $r_u$ is a positive parameter that corresponds to the growth rate, and $\epsilon_{u, n}$ are iid. lognormal random variables such that $\log \epsilon_{u, n} \sim \normal[0, \sigma_u^2]$.
Measurements of the population are obtained via the density

\begin{equation*}
\log Y_{u, n} \sim \normal \left[\log X_{u, n}, \, \tau_u^2\right]
\end{equation*}

\noindent where $\tau_u$ is a positive variance parameter.
...
% despite Theorem~\ref{theorem:pif} providing stronger theoretical support of the PIF algorithm.
...
% \eic{IS THERE A REASON THAT results0, results1, results2 ARE READ FROM SPECIFIC rds FILES NOT USING BAKE/STEW?}
% \jwc{YES. TO HELP SPEED UP COMPUTATIONS, I USED THE BATCHTOOLS PACKAGE, WHICH HELPED TO ULTRA-PARALLIZATION. IT CANNOT READILY BE USED WITH BAKE / STEW. I PUSHED THE DATA FILES THOUGH, THEY ARE SMALL.}
...
@
\caption{\label{fig:gomp}Comparison of the MPIF and PIF algorithms for fitting the stochastic Gompertz population model. 
The solid horizontal line shows the true maximum likelihood, determined via the Kalman filter and a numeric optimizer, an intractable approach for high-dimensional parameter spaces $(U > 100)$, so in these cases the dashed line indicates the likelihood at the data-generating parameters. 
...
\section{Example: Measles in the United Kingdom} 
...
The total population size, $\pop^{(u)}(t)$, is treated as known, interpolated from census data, and we use this constraint to avoid explicit specification of $R^{(u)}_t$.
...
% \eic{FOR COMPATIBILITY WITH R, AND ARGUABLY WITH THE NAME EULERMULTINOM, I'VE TREATED DELTA AS AN ADDITIONAL ARGUMENT RATHER THAN INCLUDING IT IN THE RATES}
% \jwc{Sounds good to me, I'll check this with Aaron.}
\begin{align*}
...
\begin{align*}
	p_0 = \exp\big\{ -(\mu_1 + \mu_2)\eulerstep \big\} \qquad p_i = \frac{\mu_i}{\mu_1+\mu_2}\big(1-p_0\big).
\end{align*}
...
\end{align*}
where $\delta$ is the Dirac delta function, $b^{(u)}(t)$ is the births per year at time $t$ linearly interpolated from annual data, $\tau_d$ is the delay in transitions from $B$ to $S$, $\tau_y = 1$ year, and $\tau_c$ is the school admission day, i.e., the 251st day of the year. 
We set $\tau_d = 4$ years to describe the age at which individuals typically enter a high-transmission environment.

The rate at which individuals enter state $E$ is $\mu^{(u)}_{SE}(t) = \bar{\mu}^{(u)}_{SE}(t) \, d\Gamma^{(u)}(t)/dt$, where $\bar{\mu}^{(u)}_{SE}(t)$ is the mean rate given by
\begin{align*}
  \bar{\mu}^{(u)}_{SE}(t) =
    \frac{
...
\begin{align*}
	...
	\end{cases}
\end{align*}
where $p = 0.7589$ is the proportion of the year occupied by the school term, and
\begin{align*}
	\beta^{(u)}_0 = R^{(u)}_0\big( 1-\exp\big\{ -(\mu^{(u)}_{IR}(t)+\mu^{(u)}_{ID})\big\}\big).
\end{align*}
The remaining transition rates are assumed to be constant:
\begin{align*}
	\mu^{(u)}_{EI}(t) = \sigma^{(u)} \qquad \mu^{(u)}_{IR}(t) = \gamma^{(u)} \qquad \mu^{(u)}_{SD} = \mu^{(u)}_{ED} = \mu^{(u)}_{ID} = 0.02 \, \text{yr}^{-1}.
\end{align*}
...
\end{enumerate}
Each model interpolates the birthrate using splines instead of linear interpolation.
...
By 200 iterations, we see that MPIF yields maximum log-likelihoods 70 to 105 units higher than PIF for the $\iota$-shared and $c$-shared models, or about 4 to 5 units per city. 
MPIF has about half the advantage for the 7-shared model by 200 iterations, reflecting how MPIF provides the greatest benefit when there are more unit-specific parameters. 
...
gg <- results_in |>
  ...
  #theme(legend.position = 'inside', legend.position.inside = c(0.13, 0.15))
...
print(gg)
@
\caption{\label{fig:measles1} Log-likelihoods yielded by the PIF and MPIF algorithms for the mechanistic measles model to the UK data. 
...
The issue of particle depletion when estimating the posterior distribution of parameter values using a particle filter is well known (see, for instance, Section~1.2 of \citet{chen24}). 
...
% \begin{description}
% 
% \item[Supplemental Text:] This file contains a detailed description of all simulation studies performed in the article. (PDF)
% 
% \end{description}
% 
% \FloatBarrier
