...
\begin{theorem}\label{theorem:pif}
  ...
  Assume conditions \ref{assumption:1}--\ref{assumption:kernel} of Appendix~\ref{sec:assumptions} are satisfied, and write $\hat{\theta} = \argmax_{\theta} L(\theta;\mathbf{y}^*)$ from Eq.~\ref{eq:likedef}.
  ...
  $$
  E\bigg[\Big\vert\frac{1}{J}\sum_{i=1}^J \Theta_j^{(M)} - \hat{\theta}\Big\vert_2\bigg] \leq \frac{C_M}{\sqrt{J}} + \epsilon_M
  $$
\end{theorem}
...
\end{align*}
In this representation, $f(\mathbf{y}^*; \theta)$ is the likelihood function (Eq.~\ref{eq:likedef}), and $\pi_0(\theta)$ is the original prior distribution for $\theta$.
...
\noindent where $K_u$ is the carrying capacity for the population in unit $u$, $r_u$ is a positive parameter that corresponds to the growth rate, and $\epsilon_{u, n}$ are iid. log-normal random variables such that $\log \epsilon_{u, n} \sim \normal[0, \sigma_u^2]$.
...
@
\caption[MPIF compared to PIF on Gompertz population model.]{\label{fig:gomp}Comparison of the MPIF and PIF algorithms for fitting the stochastic Gompertz population model. 
...
print(gg)
@
\caption[MPIF comparison using mechanistic model for UK measles incidence data.]{\label{fig:measles1} Log-likelihoods yielded by the PIF and MPIF algorithms for the mechanistic measles model to the UK data. 
