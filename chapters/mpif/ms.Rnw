...
However, previous studies have leveraged simple linear models for panel data and have argued for the importance of fitting models to entire collections of data rather to independent time series \citep[e.g.,][]{wiens14}.
A more plausible explanation is that the lack of examples may be indicative that scientists have struggled, largely unsuccessfully, to use existing statistical methodology.
